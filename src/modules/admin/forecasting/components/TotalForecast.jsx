// src/modules/admin/forecasting/components/TotalForecast.jsx
...
import { getForecastProjectAll, getPrescriptiveProject } from "../../../../services/forecastService";

export function TotalForecast() {
    ...
    useEffect(() => {
        ...
        getForecastProjectAll()
            .then(async (response) => {
                const forecast = response.forecasts[0];

                const byDate = (a, b) =>
                    new Date(a.date).getTime() - new Date(b.date).getTime();
                const rawHist = [...forecast.history].sort(byDate);
                const rawFct = [...forecast.forecasting].sort(byDate);
                ...
                const forecastData = rawFct.map((d) => ({
                    originalDate: d.date,
                    date: formatSemester(d.date),
                    value: d.value,
                    lower: d.lower,
                ...
                setProjectionData([...history, ...forecastData]);
                setLoading(false);
            ...
            .catch((e) => {
                setError(e.message);
                setLoading(false);
                toast.error("Error al cargar los datos de predicción total.");
            });
    }, []);
