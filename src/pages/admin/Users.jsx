...
const Users = () => {
  ...
  const filteredUsers = users.filter((user) => {
    const matchesSearch =
      user.nombre.includes(searchTerm) || user.correo.includes(searchTerm);
        ...
        <div class="text-left space-y-2">
          ...
          ${
            !esPostulacion
              ...
              : `<p><strong>Estado:</strong> <span style="color: #f59e0b;">En espera de aprobación</span></p>`
          }
        </div>
      `,
      icon: "info",
    ...
    try {
      ...
      await Swal.fire({
        ...
        text: `Usuario ${user.nombre} ${
          esHabilitado ? "deshabilitado" : "habilitado"
        } correctamente`,
        icon: "success",
        confirmButtonColor: "#0891b2",
        timer: 2000,
    ...
    <AdminLayout title="Usuarios">
      <div className="space-y-4 md:space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
          ...
          />
          <EstadoFilter value={selectedEstado} onChange={(e) => setSelectedEstado(e.target.value)} />
          <RolFilter value={selectedRol} onChange={(e) => setSelectedRol(e.target.value)} />
        </div>

        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
          <QuantitySelector value={itemsPerPage} onChange={(e) => setItemsPerPage(Number(e.target.value))} />
        ...
        <UserTable users={filteredUsers.slice(0, itemsPerPage)} onView={handleViewUser} onToggleEstado={handleToggleEstado} />
        ...
        {pendingApplications.length > 0 && (
          <>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
              ...
              />
              <p className="text-xs sm:text-sm text-gray-600 text-center sm:text-right">
                Mostrando {Math.min(filteredPendingApplications.length, pendingItemsPerPage)} de{" "}
                {filteredPendingApplications.length} postulaciones
              </p>
            </div>
          </>
        )}
