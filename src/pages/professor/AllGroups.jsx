...
import { useAuth } from "../../contexts/AuthContext"; // ✅ Cambio aquí
...
const AllGroups = () => {
  ...
  const { userData } = useAuth(); // ✅ Cambio aquí: usar userData en lugar de user
  ...
  useEffect(() => {
    const loadGroups = async () => {
      // No hacer nada si no hay userData
      ...
      setError("");
      try {
        const list = await listarGruposPorUsuario(userData.id_usuario); // ✅ Cambio aquí
    ...
    loadGroups();
  }, [userData]); // ✅ Cambio aquí: depender de userData

  const handleQRCode = (group) => {
    // Aquí puedes implementar la lógica real para generar el QR
    console.log("Generar QR para grupo:", group);
    alert(`Generar QR para el grupo: ${group.nombre_grupo || group.nombre}`);
  };
