...
const StatusBadge = ({ status }) => {
  const statusConfig = {
    "en revisión": { bg: "bg-yellow-100", text: "text-yellow-800", border: "border-yellow-200" },
    "corregido": { bg: "bg-blue-100", text: "text-blue-800", border: "border-blue-200" },
  ...
  const config = statusConfig[status?.toLowerCase()] || statusConfig["en revisión"];
...
const ApprovedProjectCard = ({
  ...
  return (
    <div
      className="bg-gray-200 rounded-2xl shadow-md hover:shadow-lg transition-shadow overflow-hidden cursor-pointer"
      onClick={onClick}
    >
      <div className="flex flex-col md:flex-row p-6 gap-6">
        ...
        <div className="flex-shrink-0">
          {logo ? (
            <img src={logo} alt={title} className="w-24 h-24 rounded-xl object-cover" />
          ) : (
            <div className="w-24 h-24 rounded-xl bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-700 flex items-center justify-center">
